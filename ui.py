...
def sidebar(df):
    with st.sidebar:
        ...
        st.caption(
            ...
            f"| Usunięcia: {len(st.session_state.to_delete)}"
        )
...
def product_list(view, queue_delta):
    ...
    # stan edycji – ID aktualnie edytowanego produktu
    ...
    for idx, (_, row) in enumerate(view.iterrows()):
        ...
        with st.expander(expander_title, expanded=False):
            ...
            if c3.button("❌", key=f"del_{row['ID']}_{idx}"):
                ...
                st.session_state.df_cache = st.session_state.df_cache[
            ...
            if st.session_state.editing_id == row["ID"]:
                with st.form(f"edit_form_{row['ID']}"):
                    produkt = st.text_input("Nazwa produktu", value=row["Produkt"])
                    firma = st.text_input("Firma", value=row["Firma"])
                    typ = st.text_input("Typ", value=row["Typ"])
                    nr_ser = st.text_input("Numer seryjny", value=row["Nr seryjny"])
                    lokalizacja = st.text_input("Lokalizacja", value=row["Lokalizacja"])
                    stan = st.number_input("Stan", min_value=0, step=1, value=int(row["Stan"]))

                    csave, ccancel = st.columns(2)
                    submitted = csave.form_submit_button("💾 Zapisz zmiany")
                    canceled = ccancel.form_submit_button("❌ Anuluj")
                    ...
                    if submitted:
                        ...
                        st.session_state.require_full_save = True
                        st.session_state.editing_id = None
                        st.success(f"✏️ Zaktualizowano produkt: {produkt}")
                        st.rerun()

                    if canceled:
                        st.session_state.editing_id = None
                        st.info("Edycja anulowana.")
                        st.rerun()
            else:
...
def deleted_items_history(undo_delete_by_id):
    ...
    if st.session_state["historia_usuniec"]:
        for hist_item in reversed(st.session_state["historia_usuniec"]):
            ...
            with col1:
                st.write(
                    f"**{hist_item.get('Produkt','')}** — {hist_item.get('Firma','')} "
                    f"({hist_item.get('Typ','')})"
                )
            with col2:
